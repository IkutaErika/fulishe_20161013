...
//        mBean.url=url;
    ...
    public static void downloadImg(UserBean user,Context context,ImageView imageView,String thumb){
        setAvatar(user,I.DOWNLOAD_AVATAR_URL+thumb,context,imageView);
    }
    ...
    public static String getAvatarUrl(UserBean user){
      return I.SERVER_ROOT+I.REQUEST_DOWNLOAD_AVATAR+I.QUESTION+ I.NAME_OR_HXID+"="
              +user.getMuserName()+I.AND+I.AVATAR_TYPE+"="+user.getMavatarPath()+I.AND+
              I.Avatar.AVATAR_SUFFIX+"="+user.getMavatarSuffix()+I.AND+"width=200"+I.AND+"height=200";
    }
    public static void setAvatar(UserBean user, String url, Context context, ImageView imageView){
        ImageLoader.build(getAvatarUrl(user))
                .imageView(imageView)
                .showImage(context);
        L.i(getAvatarUrl(user));
    }
}
